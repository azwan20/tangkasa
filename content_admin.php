...
<style>
    ...
    .editBtnContent, .saveBtnContent, .cancelBtnContent{
    ...
    <main>
        ...
        ?>
                <section class="content-item" <?php echo $id; ?>>
                    <h2 class="judulContent"><?php echo $judulContent; ?></h2>
                    <p class="keterangan">
                        <?php echo $keterangan; ?>
                    </p>
                    <input type="hidden" class="idInputContent" value="<?php echo $id; ?>">
                    <div style="margin: 20px 0 30px; color: #fff; height: 60px">
                        <button class="editBtnContent" onclick="editCardContent(this)" style="color: #fff;">
                        ...
                        <button class="saveBtnContent" onclick="saveCardContent(this)" style="display: none; color: #fff;">Save</button>
                        <button class="cancelBtnContent" onclick="cancelCardContent(this)" style="display: none; color: #fff;">Cancel</button>
                    </div>
                </section>
            <?php
            }
...
<script>
    function editCardContent(button) {
        var cardContent = button.closest('.content-item');
        var titleElement = cardContent.querySelector('.judulContent');
        var keteranganElement = cardContent.querySelector('.keterangan');

        titleElement.innerHTML = '<input type="text" class="judulInputContent" value="' + cardContent.querySelector('.judulContent').innerText + '">';
        keteranganElement.innerHTML = '<input type="text" class="keteranganInput" value="' + cardContent.querySelector('.keterangan').innerText + '">';

        cardContent.querySelector('.saveBtnContent').style.display = "inline";
        cardContent.querySelector('.cancelBtnContent').style.display = "inline";
        button.style.display = "none";
    }

    function saveCardContent(button) {
        var cardContent = button.closest('.content-item');
        var newTitle = cardContent.querySelector('.judulInputContent').value;
        var newKeterangan = cardContent.querySelector('.keteranganInput').value;
        var id = cardContent.querySelector('.idInputContent').value;

        var xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                cardContent.querySelector('.judulContent').innerHTML = newTitle;

                cardContent.querySelector('.keterangan').innerHTML = newKeterangan;

                cardContent.querySelector('.saveBtnContent').style.display = "none";
                cardContent.querySelector('.cancelBtnContent').style.display = "none";
                cardContent.querySelector('.editBtnContent').style.display = "inline";
            }
        };

        xhr.open("POST", "save_content.php", true);
        xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhr.send("id=" + id + "&judulContent=" + newTitle + "&keterangan=" + newKeterangan);
        location.reload();
    }

    function cancelCardContent(button) {
        var cardContent = button.closest('.content-item');
        cardContent.querySelector('.judulContent').innerHTML = cardContent.querySelector('.judulContent').value;
        cardContent.querySelector('.keterangan').innerHTML = cardContent.querySelector('.keterangan').value;
