...
<div class="editHome d-flex">
    <button class="d-flex" onclick="editInfo()">
    ...
    <button class="d-flex" onclick="saveInfo()" id="saveBtnInfo" style="display: none !important;">
        Save
    </button>
    <button class="d-flex" onclick="cancelInfo()" id="cancelBtnInfo" style="display: none !important;">
        Cancel
    </button>
</div>
<hr>
<div class="d-flex info" style= "flex-wrap: wrap;">
    ...
    ?>
            <div class="infoItem" style="width: 250px; box-sizing: border-box; margin-bottom: 40px;">
                ...
                <p id="judul" style="text-align: center;"><?php echo $judul; ?></p>
            </div>
        <?php
            $counter++;
            if ($counter % 3 == 0) {
...
<script>
    ...
    function editInfo() {
        ...
        gambarElement.innerHTML = '<input type="text" id="gambarInput" value="<?php echo $gambar; ?>">';
        judulElement.innerHTML = '<input type="text" id="judulInput" value="<?php echo $judul; ?>">';

        document.getElementById("saveBtnInfo").style.display = "inline";
        document.getElementById("cancelBtnInfo").style.display = "inline";
    }

    function saveInfo() {
        var newGambar = document.getElementById("gambarInput").value;
        var newJudul = document.getElementById("judulInput").value;

        var xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                document.getElementById("gambar").src = newGambar;
                document.getElementById("judul").innerText = newJudul;

                document.getElementById("saveBtnInfo").style.display = "none";
                document.getElementById("cancelBtnInfo").style.display = "none";
            }
        };

        xhr.open("POST", "save_info.php", true);
        xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhr.send("gambar=" + newGambar + "&judul=" + newJudul);
    ...
    function cancelInfo() {
        document.getElementById("gambar").innerHTML = "<?php echo $gambar; ?>";
        document.getElementById("judul").innerHTML = "<?php echo $judul; ?>";
